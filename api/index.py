...
async def handle_chat(payload: ChatPayload):
    ...
    UI_LANGS = {
        ...
        'de': {'err_city': "Bitte geben Sie eine Stadt an.", 'not_found': "Keine Hotels gefunden.", 'found': "Optionen gefunden", 'hotel': "HOTEL", 'verified': "✓ Überprüft", 'show_prices': "Preise anzeigen", 'advice_title': "💡 Tipp für", 'show_more': "Zeige", 'hotels_more': "weitere aus нашей базы ⬇️", 'show_all': "🗺️ Alle Optionen auf der Karte finden", 'more_cmd': "Mehr", 'filter_msg': "Gefiltert nach Ihrer Anfrage!", 'found_hotel': "🎯 Hier ist die gesuchte Unterkunft:", 'not_found_alt': "Die genaue Unterkunft <b>{hotel}</b> wurde не найден. Hier sind Alternativen in der Nähe:"},
    ...
    try:
        # УЛУЧШЕННЫЙ АНАЛИЗАТОР (С ПРИМЕРАМИ ОБУЧЕНИЯ)
        analyzer_prompt = f"""
        Task: Extract travel info from user query. Respond ONLY with valid JSON.
        
        Examples:
        Query: 'NapCastle Caledonian Park' -> {{"city": null, "hotel": "NapCastle Caledonian Park", "filter": null, "wants_more": false}}
        Query: 'London with pool' -> {{"city": "London", "hotel": null, "filter": "pool", "wants_more": false}}
        Query: 'Италия' -> {{"city": "Rome", "hotel": null, "filter": null, "wants_more": false}}

        CRITICAL: If the hotel name starts with 'Nap', do NOT assume the city is Naples. Treat the whole name as a hotel.
        User Query: '{msg}'
        """
        ...
        # --- СНАЙПЕР ---
        if hotel_name:
            # Если ИИ нашел название отеля, ищем его максимально широко
            ...
            if redis_db:
                ...
                if cached_hotel_raw:
                    try:
                        ...
                        h_id = c_hotel.get('id'); h_name = c_hotel.get('name'); h_desc = c_hotel.get('desc')
                        link = f"https://www.stay22.com/allez/booking/{h_id}?aid={STAY22_AID}"
                        html = f"""<div style="font-family: sans-serif; width: 100%; color: #1a1a1a; padding: 10px 0;"><div style="max-width: 1000px; margin: 0 auto;"><h2 style="font-size: 20px; color: #003580;">{t['found_hotel']}</h2><div style="background: #ffffff; border: 1px solid #e7e7e7; border-radius: 8px; padding: 15px; display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 15px;"><div style="flex: 1; min-width: 280px;"><div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;"><span style="background: #003580; color: #fff; font-size: 11px; padding: 2px 8px; border-radius: 4px;">{t['hotel']}</span><span style="color: #008009; font-size: 12px; font-weight: 700;">{t['verified']}</span></div><div style="font-size: 18px; font-weight: 700; color: #006ce4; margin-bottom: 8px;">{h_name}</div><div style="font-size: 13px; color: #4a4a4a; line-height: 1.5;">{h_desc}</div></div><div style="text-align: right; min-width: 150px;"><a href="{link}" target="_blank" style="background: #006ce4; color: #ffffff; text-decoration: none; padding: 12px 24px; border-radius: 4px; font-size: 14px; font-weight: 600; display: inline-block; width: 100%; text-align: center;">{t['show_prices']}</a></div></div></div></div>"""
            ...
            try:
                ...
                # Более гибкий выбор: ищем совпадение в названии
                for r in results:
                    if r.get('dest_type') not in ['city', 'region', 'country', 'airport'] or 'name' in r:
                        target_hotel = r
                        break
                        
                if target_hotel:
                    h_id = str(target_hotel.get('dest_id') or target_hotel.get('hotel_id') or '')
                    if h_id:
                        h_name = target_hotel.get('name', hotel_name.title())
                        g_prompt = f"Write 1 short sentence describing '{h_name}' in '{user_lang.upper()}'. JSON: {{\"d\": \"text\"}}"
                        try:
                        ...
                        if redis_db: redis_db.set(hotel_db_key, json.dumps({"id": h_id, "name": h_name, "desc": h_desc}))
                        link = f"https://www.stay22.com/allez/booking/{h_id}?aid={STAY22_AID}"
                        html = f"""<div style="font-family: sans-serif; width: 100%; color: #1a1a1a; padding: 10px 0;"><div style="max-width: 1000px; margin: 0 auto;"><h2 style="font-size: 20px; color: #003580;">{t['found_hotel']}</h2><div style="background: #ffffff; border: 1px solid #e7e7e7; border-radius: 8px; padding: 15px; display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 15px;"><div style="flex: 1; min-width: 280px;"><div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;"><span style="background: #003580; color: #fff; font-size: 11px; padding: 2px 8px; border-radius: 4px;">{t['hotel']}</span><span style="color: #008009; font-size: 12px; font-weight: 700;">{t['verified']}</span></div><div style="font-size: 18px; font-weight: 700; color: #006ce4; margin-bottom: 8px;">{h_name}</div><div style="font-size: 13px; color: #4a4a4a; line-height: 1.5;">{h_desc}</div></div><div style="text-align: right; min-width: 150px;"><a href="{link}" target="_blank" style="background: #006ce4; color: #ffffff; text-decoration: none; padding: 12px 24px; border-radius: 4px; font-size: 14px; font-weight: 600; display: inline-block; width: 100%; text-align: center;">{t['show_prices']}</a></div></div></div></div>"""
                        return JSONResponse(content={"reply": html})
        ...
        db_key = f"v16:booking:{city_en}:{user_lang}"; lock_key = f"lock:v16:{city_en}:{user_lang}"
        full_list = []
        if redis_db:
        ...
        if redis_db and not redis_db.get(lock_key):
            ...
            if new_items:
                g_prompt = f"Data: {json.dumps(new_items)} in {city_en}. Write JSON: {{\"adv\": \"hack\", \"cats\": [ {{\"id\": \"id\", \"n\": \"name\", \"cat\": \"{t['hotel']}\", \"d\": \"desc\"}} ]}} in {user_lang.upper()}."
                ...
                try:
                    ...
                    last_adv = new_data.get('adv', ''); raw_cats = new_data.get('cats', [])
                    if isinstance(raw_cats, dict): raw_cats = [raw_cats]
                    for h in raw_cats:
                        if isinstance(h, dict): h['advice'] = last_adv; full_list.insert(0, h)
                    if redis_db and full_list:
                        redis_db.set(db_key, json.dumps(full_list)); redis_db.set(lock_key, "1", ex=86400)
                except: pass
        if not full_list: return JSONResponse(content={"reply": t['not_found']})
        to_show = []; is_filtered = False
        if user_filter and len(full_list) > 3:
            f_prompt = f"Find ids matching '{user_filter}' in {json.dumps([{'id': h.get('id'), 'n': h.get('n')} for h in full_list[:30]])}. JSON: {{\"ids\": []}}"
            f_res = requests.post("https://api.groq.com/openai/v1/chat/completions", headers=headers, json={"model": "llama-3.1-8b-instant", "messages": [{"role": "user", "content": f_prompt}], "response_format": {"type": "json_object"}}, timeout=5)
            try:
                filtered_ids = json.loads(f_res.json()['choices'][0]['message']['content']).get('ids', [])
                to_show = [h for h in full_list if h.get('id') in filtered_ids][:5]; is_filtered = True
            except: to_show = full_list[:5]
        elif wants_more and len(full_list) > 5: to_show = random.sample(full_list, min(5, len(full_list)))
        if not to_show: to_show = full_list[:5]
        hidden_count = len(full_list) - len(to_show)
        subtitle = f"<span style='color: #008009;'>✨ {t['filter_msg']}</span>" if is_filtered else f"{len(full_list)} {t['found']}"
        html = f"<div style='font-family: sans-serif; width: 100%; color: #1a1a1a;'><div style='max-width: 1000px; margin: 0 auto;'>"
        if hotel_name and not target_hotel:
            html += f"<div style='background: #fff3cd; color: #856404; border: 1px solid #ffeeba; border-radius: 8px; padding: 12px; margin-bottom: 15px;'>{t['not_found_alt'].format(hotel=hotel_name.title())}</div>"
        html += f"<h2 style='font-size: 20px; font-weight: 700; color: #003580;'>{city_en.capitalize()}: {subtitle}</h2>"
        for h in to_show:
            if not isinstance(h, dict): continue
            link = f"https://www.stay22.com/allez/booking/{h.get('id', '')}?aid={STAY22_AID}"
            html += f"<div style='background: #ffffff; border: 1px solid #e7e7e7; border-radius: 8px; padding: 15px; margin-bottom: 12px; display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 15px;'><div style='flex: 1; min-width: 280px;'><div style='display: flex; align-items: center; gap: 8px; margin-bottom: 6px;'><span style='background: #003580; color: #fff; font-size: 11px; padding: 2px 8px; border-radius: 4px;'>{h.get('cat', t['hotel'])}</span><span style='color: #008009; font-size: 12px;'>{t['verified']}</span></div><div style='font-size: 18px; font-weight: 700; color: #006ce4;'>{h.get('n', 'Hotel')}</div><div style='font-size: 13px; color: #4a4a4a;'>{h.get('d', '')}</div></div><div style='text-align: right; min-width: 150px;'><a href='{link}' target='_blank' style='background: #006ce4; color: #ffffff; text-decoration: none; padding: 12px 24px; border-radius: 4px; display: inline-block; width: 100%; text-align: center;'>{t['show_prices']}</a></div></div>"
        if len(to_show) > 0 and isinstance(to_show[0], dict) and to_show[0].get('advice') and not is_filtered:
            html += f"<div style='background: #ebf3ff; border: 1px solid #003580; border-radius: 8px; padding: 16px; margin: 20px 0;'><div style='font-size: 14px; color: #003580;'><b>{t['advice_title']} {city_en.capitalize()}:</b> {to_show[0]['advice']}</div></div>"
        ...
        if hidden_count > 0:
            ...
            html += f"<button onclick=\"{btn_action}\" style='display: block; width: 100%; padding: 16px; margin-bottom: 12px; background: #ffffff; color: #006ce4; border: 1px solid #006ce4; border-radius: 8px; font-weight: 700; cursor: pointer;'>{t['show_more']} {hidden_count} {t['hotels_more']}</button>"
        html += f"<a href='{all_link}' target='_blank' style='display: block; width: 100%; text-align: center; padding: 16px; text-decoration: none; border-radius: 8px; font-weight: 700; background: #003580; color: #ffffff;'>{t['show_all']}</a></div></div>"
        return JSONResponse(content={"reply": html})
    except Exception as e:
        return JSONResponse(content={"reply": f"Technical error: {str(e)}"})
